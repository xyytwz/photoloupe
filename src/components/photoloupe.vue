...
<template>
    <div class="comp-magnifier" @mousemove.stop="mouseOver" @mouseout.stop="mouseOut">
        ...
        <!--放大镜图标-->
        <img class="glass" src="../assets/g-glass.png" width="17px" height="17px">
        <!--放大镜捕获位置-->
        <span class="ks-imagezoom-lens"  :class="{show:isShow}" :style="{'height':toPx(height/magnification),'width':toPx(width/magnification),'left':toPx(position(x,width)),'top':toPx(position(y,height))}"></span>
        <!--放大效果展示层-->
        <div class="magnifier" :class="{show:isShow}" :style='{"width":toPx(width),"height":toPx(height),"left":toPx(width+10)}'>
            <img :src="src" :height="height*magnification" :width="width*magnification" :style="{'top': toPx(-position(y,height)*magnification),'left':toPx(-position(x,width)*magnification)}">
        </div>
    </div>
</template>

<script>
    import blankImage from "../assets/defaultImg.png";
    export default {
        name: "photo-loupe",
        props: {
            //图片路径
